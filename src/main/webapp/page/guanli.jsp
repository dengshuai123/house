...
<%
  ...
  if(o==null){
    out.print("<script>alert('你还没有登入');location.href='login.jsp';</script>");
  }
%>
...
<TABLE class=house-list>
  <TBODY>
  <c:forEach items="${houseExtPageInfo.list}" var="house">
  <TR>
    ...
    <TD>
      <DL>
        ...
        <DD>${house.dname}区${house.sname}街,${house.floorage}平米,户型：${house.tname}<BR>联系方式：${house.contact} </DD>
      </DL>
    </TD>
    <TD class=house-type>
      <LABEL class=ui-green>
        <INPUT onclick="location.href='upShowHouse?id=${house.id}';" value="修    改" type=button name=search>
      </LABEL>
    </TD>
    <TD class=house-price>
      <LABEL class=ui-green>
        <INPUT onclick="delHouse(this,${house.id})" value="删    除" type=button name=search>
      </LABEL>
    </TD>
  </TR>
  </c:forEach>
...
<UL>
  ...
  </UL><SPAN class=total>${houseExtPageInfo.pageNum}页/${houseExtPageInfo.pages}页</SPAN> </DIV></DIV>
